function y3=alg_srcmf(Y,Sd,St,cv_setting,nr_fold,left_out,use_WKNKN,K,eta,use_W_matrix)
%alg_cmf predicts DTIs based on the algorithm described in the following paper: 
%Gao LG, Yang MY, Wang JX. Collaborative matrix factorization with soft regularization for drug-target interaction prediction. 
%JOURNAL OF COMPUTER SCIENCE AND TECHNOLOGY 36(2): 310¨C322 Mar. 2021. DOI 10.1007/s11390021-0844-8
%
% INPUT:
...
%  y3:  prediction matrix
    ...
    % get best parameters
    [k,lambda_l,lambda_d,lambda_t,num_iter] = alg_srcmf_parest(Y,Sd,St,cv_setting,nr_fold,left_out,use_WKNKN,K,eta,use_W_matrix);
    ...
    W = ones(size(Y));
    W(test_ind) = 0;
    [A,B] = alg_srcmf_predict(Y,A,B,Sd,St,lambda_l,lambda_d,lambda_t,num_iter,W);
